...
Fonctionnalité: Opérations de collecte de données pour construire un SIP consommable par VITAM
  ...
  Scénario: Test OK d'un ingest en SEDA 2.2 avec un profil d'archivage
    ...
    Et que j'importe le sip suivant data/SIP_WARNING/ZIP/WARNING_SIPWithAP_mail_Seda2.2.zip
    Et je recherche le journal des opérations
...
"""
  ...
  Scénario: Test OK d'un ingest en SEDA 2.2 avec un profil d'unité archivistique
    Quand j'initialise une transaction
    Et que j'importe le sip suivant data/SIP_WARNING/ZIP/WARNING_SIPwith2AUP_mail_Seda2.2.zip
    Et je recherche le journal des opérations
    Alors le statut final du journal des opérations est WARNING
    Et je nomme l'identifiant de l'opération OperationId1
    Quand j'utilise la requête suivante
...
"""
  ...
  Scénario: Import dans collect d'un SIP KO en SEDA 2.1 déclarant des champs 2.2
    Quand j'initialise une transaction
    Et que j'importe le sip suivant data/SIP_KO/ZIP/SIP_2.1_WITH_FIELD_2.2.zip
    Et je recherche le journal des opérations
  ...
  Scénario: Test d'un ingest en SEDA 2.2
    ...
    Et que j'importe le sip suivant data/SIP_OK/ZIP/OK_SIP_FULL_SEDA2.2.zip
    Quand je recherche le journal des opérations
    Alors le statut final du journal des opérations est OK
    Quand j'utilise le fichier de requête suivant data/queries/select_seda2.2_fields_any_operation.json
    Et je recherche les unités archivistiques de la transaction
    Alors le nombre de résultat est 1
    Quand j'utilise le fichier de requête suivant data/queries/select_logbook_operation_by_id.json
    Et je recherche les journaux d'opération
    Alors les metadonnées sont
      | evDetData        |     "{\n  \"EvDetailReq\" : \"SIP SEDA 2.2\",\n  \"EvDateTimeReq\" : \"2016-10-18T14:52:27\",\n  \"ArchivalAgreement\" : \"ArchivalAgreement0\",\n  \"ServiceLevel\" : null,\n  \"AcquisitionInformation\" : \"AcquisitionInformation0\",\n  \"LegalStatus\" : \"Private Archive\"\n}" |
