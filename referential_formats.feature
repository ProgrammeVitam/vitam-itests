...
Fonctionnalité: gestion des données référentielles (vérification, import et recherche)
  ...
  Contexte: Avant de lancer cette suite de test, je présuppose que le référentiel DROID_SignatureFile_V121 est chargé (_init.feature)
  ...
  Scénario: Vérification et import des formats OK, recherche par id OK, rechercher par name OK
    Quand je vérifie le fichier nommé data/formats/DROID_SignatureFile_V121.xml pour le référentiel FORMATS
    Alors les metadonnées sont
      | Code           | 200                                     |
  ...
  Scénario: Reimport format version V121
    Quand je vérifie le fichier nommé data/formats/DROID_SignatureFile_V121.xml pour le référentiel FORMATS
    Alors les metadonnées sont
      | Code           | 200                                     |
    ...
    Quand j'utilise le fichier de requête suivant data/queries/select_format_all.json
    Et je recherche les données dans le référentiel FORMATS
    Alors le nombre de résultat est 2521
